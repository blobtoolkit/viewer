...
class Dataset extends React.Component {
  ...
  render(){
    ...
    return (
      <div className={css}>
        <a data-tip data-for='load-dataset' className={styles.cover} onClick={()=>this.props.onDatasetClick(this.props.id)}>
          <h3>{this.props.meta.name}</h3>
          <span className={styles.menu_subtitle}>{accession}<br/><em>{taxon}</em></span>
        </a>
        <span className={styles.menu_count}>{records.toLocaleString() + ' ' + record_type}</span>
